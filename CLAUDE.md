...
- **New vacpack brokers (added 2026-08-13, from the 2026-08 source-research swarm — candidates in `research/new-source-candidates-2026-08.md`):** travel-bargains (AC/Orlando/San Diego, Magna Timeshare Software), genesis-group (Vegas/Branson, WooCommerce), branson-reservations-center (6 Branson pkgs), vallarta-sales (Paradise Village Nuevo Vallarta MX), goodtime-entertainment (one $199 pkg redeemable at 16 US resorts), time4avacation (Williamsburg/Hilton Head/Branson), vacation-branson, poconomo (Poconos/Exploria), williamsburg-tickets, gotspot (Global Solutions FamPak — low incremental value, overlaps payvibe). booksi + provacationgroup need a Playwright path (client-rendered). **Cloudflare-UA gotcha:** several of these serve a JS-challenge to non-browser UAs (branson-reservations-center emitted 0 until fixed) — add the desktop-UA `preNavigationHooks` block (copy from `branson-reservations-center.ts`) to any new Cheerio broker crawler.
