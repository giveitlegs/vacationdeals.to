...
- **KVM2** (72.60.126.238, id 1325904, created 2026-02-03) is a SEPARATE idle Traefik/Docker box. **Audited 2026-09-10 = EMPTY** — only `traefik:latest` + Hostinger's own `hermes-agent` management container; no WordPress/DB, no vhosts, no app dirs, no site. **Hosts nothing of the owner's → cleared for shutdown.** Note: stopping a Hostinger VPS still bills — cancel the plan to save money (owner billing action). See docs/NEXT-ENHANCEMENTS.md.
...
## Scrapers (66 crawlers — see docs/SCRAPING-REFERENCE.md for full details)
**56 CheerioCrawler + 10 PlaywrightCrawler** across 62 active + 4 parked sources, 66 brands, 149 destinations. (2026-08-13: +12 new broker sources added — see the "New vacpack brokers" bullet below; +252 deals.)
...
- **Parked sources** (skipped by waves): monster-vacations, govip, discount-vacation, timeshare-presentation-deals, **gotspot** (parked 2026-08-13: thegotspot.com serves EXPIRED archive deals at HTTP 200 with prices still shown — the id-range crawl ingested 180 accurate-but-unbookable "No longer available" deals from 2019–2024; deactivated + parked as a stale archive, low incremental value anyway since it overlaps payvibe's FamPak inventory). Unpark: `UPDATE sources SET status='active' WHERE scraper_key='...'`.
...
- **New vacpack brokers (added 2026-08-13, from the 2026-08 source-research swarm — candidates in `research/new-source-candidates-2026-08.md`; +252 deals):** travel-bargains (5; AC/Orlando/San Diego, Magna Timeshare Software), genesis-group (4; Vegas/Branson, WooCommerce), branson-reservations-center (6 Branson pkgs), vallarta-sales (5; Paradise Village Nuevo Vallarta MX), goodtime-entertainment (16; one $199 pkg redeemable at 16 US resorts), time4avacation (4; Williamsburg/Hilton Head/Branson), vacation-branson (2), poconomo (3; Poconos/Exploria), williamsburg-tickets (4), **gotspot (125; Global Solutions "FamPak" — deep-crawls the `/en/deals/global-solutions-llc-<1..410>` id range; low incremental value, overlaps payvibe), provacation-group (19; Duda site, SSR Cheerio, Mexico+Caribbean AI $999/$1199), booksi (59; PlaywrightCrawler — client-rendered React hub; two-phase hub-card + lander enrichment)**.
- **Broker-crawler lessons (2026-08-13, learned the hard way — apply to any new one):** (1) **Cloudflare-UA gotcha** — several serve a JS-challenge to non-browser UAs (branson-reservations-center emitted 0 until fixed); copy the desktop-UA `preNavigationHooks` block from `branson-reservations-center.ts`. (2) **`$ 199` space** — some sites render the price with a space after `$`; use `\$\s*([\d,]+)`, not `\$([\d,]+)`. (3) **Context-window bleed** — when classifying multiple `$` figures on one page, clamp each figure's context window at the nearest neighboring `$` so a package price can't inherit an adjacent fee/credit's descriptor (vacation-branson bug). (4) These promo pages dangle gift-cards/credits/rack-rates/upsells everywhere — ALWAYS verify a new crawler's stored prices in the DB against the live page before trusting it (DB spot-check caught wrong prices on time4avacation + vacation-branson this session).
...
- **"Get Paid to Go on Vacation" (`/get-paid-to-go-on-vacation`, added 2026-08-14):** a computed-at-render feature (like internal-links/hubs) powered by `apps/web/src/lib/vacation-routes.ts`. The **perk parser** is the "master table" logic: it reads each active deal's `inclusions` JSON and extracts structured perks with $ values, classifying **cash** (Visa/MC gift card = real money back) vs **credit** (resort/dining/entertainment credit = spend-there value), while EXCLUDING retail/"value"/"save"/fee/deposit figures (honesty rule — don't count marketing numbers as perks). `getVacationRoutes()` chains cities into 6 curated 1–2 week routes (`ROUTE_DEFS`) and computes honest "pay $X, get $Y back / net-cash" math live from `deals`. Components: the page + `RouteMap.tsx` (client, animated SVG route map) + sitewide `components/GetPaidWidget.tsx` (pulsing CTA). Nav has a "💸 Get Paid" CTA; in sitemap. To add/edit routes: extend `ROUTE_DEFS` (citySequence drives it; engine picks the lowest-net-cash deal per city).
...
## Lead CRM & admin security (built 2026-09-10 — see docs/CRM-LEADS-BACKEND-DESIGN.md)
- **`subscribers` is the lead source of truth.** Popup opt-ins hit `POST /api/leads`, which **upserts `subscribers`** (email unique; phone/name/source/`sms_consent` in real columns — the old phone-in-`company` hack in `data_inquiries` is retired) AND appends an **immutable `consent_records`** row (TCPA audit — NEVER delete/mutate; the admin unsubscribe is a status flag, not a delete). `data_inquiries` is now B2B-only (data-report/API inquiries). New subscriber cols (2026-09-10): `sms_consent`, `sms_consent_at`, `lifecycle` (new/engaged/customer/dormant), `notes`.
- **Admin CRM at `/admin/crm`** reads `subscribers`: search/filter (status+stage), inline lifecycle + notes, unsubscribe-as-flag. **CSV export** at `/api/admin/crm/export` (filter-aware, excludes unsubscribed unless `includeUnsub=1`, every export logged to `admin_actions`). The older `/admin/subscribers` view still shows the raw consent+inquiry merge (retire later).
- **Admin security (`/admin/security`):** opt-in **TOTP 2FA** (`admin_users.mfa_secret`/`mfa_enabled`; enforced at login ONLY once enrolled — never auto-locks the owner), **login lockout** (`failed_attempts`/`locked_until`; 8 fails → 15 min via `MAX_FAILED_ATTEMPTS`/`LOCKOUT_MINUTES` in `lib/admin/auth.ts`), **same-origin CSRF check** (`lib/admin/csrf.ts` `isSameOrigin` — apply to EVERY new admin mutation route), session cookie is **`SameSite=strict`** + httpOnly + secure. Auth helpers: `getCurrentAdmin`, `logAdminAction`, `verifyTotp`, `generateMfaSecret`.
- **Schema migrations for prod are applied as idempotent `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`** (files in `scripts/sql/`), NOT via `drizzle-kit migrate` — the deploy flow doesn't run migrations and prod may have drifted from the drizzle journal, so additive hand-SQL is the safe path. Keep `schema.ts` in sync for ORM types.
- **Email transport:** `lib/email/client.ts` `sendEmail()` prefers **SMTP (nodemailer)** when `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS` are set (Hostinger: `smtp.hostinger.com:465` secure), else falls back to **Resend** (`RESEND_API_KEY`). In-house campaign engine kept (Klaviyo declined as sender). `email_campaigns`/`email_sends` tables are scaffolded but not yet wired to actually send.
