...
export async function getPriceHistory(filters?: {
...
}): Promise<{ points: PricePoint[]; brands: BrandInfo[] }> {
  ...
  try {
    ...
    if (rows.length > 0) {
      // Group by brand + date, average price
      const grouped = new Map<string, { total: number; count: number; brandName: string; brandSlug: string }>();
      for (const r of rows) {
...
function generateMockData(days: number): { points: PricePoint[]; brands: BrandInfo[] } {
  ...
  for (let d = days - 1; d >= 0; d--) {
    ...
    for (const brand of mockBrands) {
      ...
      const seed = d * 7 + brand.slug.charCodeAt(0);
      const variance = (seededRandom(seed) - 0.5) * 80; // +/- $40
      const price = Math.max(79, Math.min(299, Math.round(base + variance)));
