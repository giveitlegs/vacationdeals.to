...
**Deactivated deals still 200 at their direct `/deals/[slug]` URL** — setting `is_active=false` removes a deal from the sitemap + listings (correct) but `(frontend)/deals/[slug]/page.tsx` fetches by slug WITHOUT an `is_active` filter, so the orphaned detail page still renders 200 (seen this session with the parked villa-group slugs). Add an `is_active` guard → `notFound()` (or `noindex` + a "deal expired" state) so dead/mispriced deals we deactivate don't linger as live pages.
